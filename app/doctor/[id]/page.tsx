...
export default async function DoctorProfilePage({
  ...
  const doctorName = profile?.full_name || `Dr. ${doctor.specialization}`;

  return (
    <main className="mx-auto max-w-2xl px-6 py-16">
      <div className="flex items-center justify-between">
        <Link href="/search" className={`text-sm ${link}`}>
          ← Back to search
        </Link>
        <ThemeToggle />
      </div>
      ...
      <div className="mt-6 rounded-2xl border border-line bg-surface p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-muted">Registration no. {doctor.license_number}</p>
          </div>
          <div className="text-right">
            <p className="text-lg font-medium text-trust-dark dark:text-trust">
              {formatFee(doctor.consultation_fee_cents)}
            </p>
        ...
        {doctor.bio && (
          <p className="mt-5 border-t border-line pt-5 text-sm leading-relaxed text-foreground/85">
            {doctor.bio}
          </p>
        )}
      </div>
      ...
      <section className="mt-10">
        <SectionHeading>Available slots</SectionHeading>
      ...
      <section className="mt-10">
        ...
        <div className="mt-4 space-y-3">
          ...
          {(reviews ?? []).map((r, i) => (
            <div key={i} className="rounded-xl border border-line bg-surface px-4 py-3.5 shadow-sm">
